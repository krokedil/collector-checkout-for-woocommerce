...
class Collector_Checkout_Post_Checkout {
	...
	function check_callback() {
		if ( strpos( $_SERVER['REQUEST_URI'], 'module/collectorcheckout/invoicestatus' ) !== false ) {
			...
			if ( isset( $_GET['InvoiceNo'] ) && isset( $_GET['InvoiceStatus'] ) ) {
				...
				if ( is_object( $order ) ) {
					...
					Collector_Checkout::log( 'Invoice status callback from Collector but we could not find the corresponding order in WC. Collector InvoiceNo: ' . wc_clean( $_GET['InvoiceNo'] ) . '. InvoiceStatus: ' . wc_clean( $_GET['InvoiceStatus'] ) . '. Order ID: ' . $order_id );
