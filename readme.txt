...
* Fix           - Fixed an infinite reload loop issue that could occur in checkout when the configured store did not support subscriptions.
* Fix           - Improved activation error handling to avoid setting orders to 'On hold' when Walley reports that the order has already been captured.
...
= 2025.08.14    - version 4.2.11 =
* Tweak         - Improved logging to be able to troubleshoot public token mismatch better.
...
= 2025.05.07    - version 4.2.8 =
* Tweak         - Added action hook 'walley_after_confirm' to allow custom actions after order confirmation is completed.
...
* Fix           - Addressed various property visibility and deprecation warnings.
* Tweak         - Added SVN to deployment script.
